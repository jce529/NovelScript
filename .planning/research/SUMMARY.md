...
- **RESOLVED (founder decision, 2026-08-25): LLM vendor = Google Gemini.** Phase 4 (AI Gateway) should plan directly against `@ai-sdk/google`'s Gemini provider — pricing/rate-limit/context-window verification for Gemini specifically still belongs in that phase's research pass, but the vendor choice itself is no longer open.
- **RESOLVED (founder decision, 2026-08-25): Toss Payments, direct integration (no PortOne abstraction layer).** Phase 5 (Real PG integration) should plan against Toss Payments' Payment Widget directly, per Stack.md's comparison table. The PortOne-specific webhook-payload re-verification note below no longer applies.
- **DEFERRED (founder decision, 2026-08-25): 선불전자지급수단 exemption compliance check.** Founder will look into this later rather than resolving it now. Both Stack and Pitfalls independently conclude the v1 design (no cash-out, no cross-creator token movement, single-merchant closed loop) likely qualifies for the exemption, and agree this is not blocking for v1 — but it remains unverified by a compliance authority. Still treat as a hard blocker before ever scoping the asset store or cash-out in a future milestone; not blocking for the v1 roadmap.
...
*Ready for roadmap: yes — LLM vendor (Gemini) and PG choice (Toss direct) confirmed by founder on 2026-08-25; PG registration lead time and 선불전자지급수단 confirmation still to be resolved by the founder outside this research pass, not blocking for v1 roadmap creation*
