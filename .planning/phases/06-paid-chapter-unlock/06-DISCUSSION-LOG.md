...
## Addendum (2026-09-08, same-day follow-up)

위 "작가 정산 원장 기록" 섹션의 "전액 그대로 작가에게" 선택은 이후 같은 날 재검토되어 CONTEXT.md D-10이 일시적으로 "비율 미정, 추가 논의 필요"로 수정되었다가, 곧이어 사용자와 다시 논의해 **90%(작가)/10%(플랫폼 수수료)** 로 확정되었다 — 단 사용자가 이 수치를 "임시 수치"라고 명시했으므로 잠정치로 문서화됨. 이 표는 최초 논의 기록으로서 그대로 남겨두되(audit trail), 현재 유효한 결정은 `06-CONTEXT.md` D-10(amended, 2nd pass)을 따른다.
